...
export function readActivatedThemes(packageContent: string): string[] {
  const parseErrors: jsonc.ParseError[] = [];
  const packageInfo = jsonc.parse(packageContent, parseErrors);

  return packageInfo.contributes.themes.map((t: Theme) =>
    path.basename(t.path, ".json")
  );
}

export function readExistingThemes(): string[] {
  return fs.readdirSync(THEMES_PATH).map((f) => path.basename(f, ".json"));
}
...
export function getThemeItems(
  ...
  for (const theme of THEMES) {
    const themeName: string = path.basename(theme, ".json");

    if (onlyConfigured && !configuredThemes.includes(themeName)) {
      continue;
    }

    const themeBase16 = getTheme(theme);

    themeItems.push({
      label: themeBase16.label,
      description: themeName,
      picked: configuredThemes.includes(themeName),
    });
  }
...
export function writeActivatedThemes(
  themeNames: string[] = []
): ThemeCompareResult {
